...
- **应用市场**：`apps.rs`（registry 拉取/安装/卸载/静默自启）+ `AppPage.tsx`（UI）。安装即覆盖写入 `apps/<id>/`，升级不单独设命令——前端对比 registry 与本地 app.json 版本号，有新版显示「升级」按钮，点击走 `install_app` 覆盖。启动应用时自动透传 `--project-path <当前项目>`（子应用可选实现该参数）。**静默自启（0.6.6 引入，0.6.7 优化）**：应用页勾选「静默自启」写入设置 `auto_start_apps`，宿主 GUI 启动时**后台线程异步拉起**（不阻塞首屏；进程列表只查一次内存匹配；所有子进程 CREATE_NO_WINDOW 无黑终端；透传 `--background` 由应用决定是否无窗口驻留）；editor-patch 首次安装默认播种勾选（用户取消不重置）；editor-patch 自身实现单实例唤起，「打开」放行由它去重。主程序不依赖任何应用的功能（解耦：编辑器控制能力/MCP 聚合服务自持于 sce_app_editor-patch）。
