...
**新增或修改任何构建/项目功能时，必须同步完成三件事，缺一不可：**
...
- **白名单构建**：code set 根下仅 `server/client/common/asset/entrance` 进产物；根级文件（init.lua、bgd_default.json、.emmyrc.json、.gitignore、doc/）各有专门流程。
