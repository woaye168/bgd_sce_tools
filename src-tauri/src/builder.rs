...
const BUILD_SUBDIRS: [&str; 5] = ["server", "client", "common", "asset", "res"];
/// 入口分界标记（标记之后为本工具生成内容）
...
pub fn build_one_file(
    ...
    // res 资源目录：.lua 声明文件不同步（它是代码），其余资源文件同步到引擎目录
...
fn parse_res_declaration(content: &str) -> Vec<(String, String)> {
    let mut out = Vec::new();
...
/// 生成 bgd_res_manifest.lua（资源清单，供 bgd_api.res 运行时查询）
pub fn generate_res_manifest(bgd_root: &Path, cfg: &BgdConfig, log: &LogFn) -> Result<()> {
    let project_name = read_project_name(bgd_root).unwrap_or_else(|_| "unknown".to_string());
    let mut entries: Vec<(String, String)> = Vec::new(); // (模块路径, 运行时路径)

    for code_set in ["libs", "game"] {
        let (dir, _) = code_set_dir(code_set, cfg);
        let src_root = cfg.abs(bgd_root, dir);
        let res_dir = src_root.join("res");
        if !res_dir.is_dir() {
            continue;
        }
        let module_prefix = Path::new(dir)
            .file_name()
            .unwrap_or_default()
            .to_string_lossy()
            .into_owned();
        ...
        for entry in fs::read_dir(&res_dir)? {
            ...
            if path.extension().is_some_and(|e| e == "lua") {
                let type_name = path.file_stem().unwrap_or_default().to_string_lossy().into_owned();
                let content = fs::read_to_string(&path)?;
                for (key, rel_path) in parse_res_declaration(&content) {
                    // 运行时路径 = 引擎目录前缀 + 声明相对路径
                    // sprites 特殊：前缀 @ProjectName（声明相对路径已含 image/sprites/）
                    let runtime_path = if type_name == "sprites" {
                        format!("@{project_name}/{rel_path}")
                    } else {
                        // 运行时路径规则（与同步目标对应）：
                        // image/particle/spine: 去掉 ui/ 或 res/ 前缀
                        // sound: 保留 res/ 前缀
                        match res_target_subdir(&type_name, code_set) {
                            Some(target) => {
                                let stripped = if type_name == "sound" {
                                    target.as_str()
                                } else {
                                    target
                                        .strip_prefix("ui/")
                                        .or_else(|| target.strip_prefix("res/"))
                                        .unwrap_or(&target)
                                };
                                format!("{stripped}/{rel_path}")
                            }
                            None => rel_path,
                        }
                    };
                    entries.push((format!("{module_prefix}.res.{type_name}.{key}"), runtime_path));
                }
            }
        }
    }

    // 生成清单文件到 libs/client/api/（bgd_api.client.res 依赖它）
    let libs_dir = cfg.abs(bgd_root, &cfg.libs_dir);
    let manifest_path = libs_dir.join("client").join("api").join("bgd_res_manifest.lua");
    let mut lines = vec![
        "-- AUTO-GENERATED BY BGD_SCE_TOOLS，请勿手改".to_string(),
        "-- 资源清单：模块路径 -> 运行时路径".to_string(),
        "return {".to_string(),
    ];
    for (k, v) in &entries {
...
pub fn build_all(bgd_root: &Path, cfg: &BgdConfig, log: &LogFn) -> Result<()> {
    ...
    generate_res_manifest(bgd_root, cfg, log)?;
