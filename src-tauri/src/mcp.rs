//! stdio MCP 聚合服务（0.5.3 场景一：AGENT 唯一 MCP 入口）。
//!
//! 运行：`bgd_sce_tools mcp`（由 MCP 客户端按需拉起，NDJSON 行协议，stdout 只写协议帧）。
//!
//! 工具集（恒定 8 个）：
//! - 本地实现（编辑器外）：editor_start / editor_stop / get_logs
//! - 在线透传 bgd_mcp_bridge：start_debug（默认 restart_last_debug）/ stop_debug /
//!   publish_project / capture_game / get_status；编辑器不在线时返回明确错误引导 editor_start
//!
//! project_path 缺省规则：参数 > bgd_sce_tools 最近项目（recent_projects.json 第一条）。
...
fn resolve_project(args: Option<&Value>) -> Result<PathBuf> {
    ...
    let dir = project::app_config_dir()?;
    let recent = project::load_recent(&dir);
    recent
        .projects
        .first()
        .map(PathBuf::from)
        .ok_or_else(|| anyhow!("未提供 project_path，且 bgd_sce_tools 无最近项目记录（请先打开过项目或显式传 project_path）"))
}

fn editor_exe_name() -> String {
    project::app_config_dir()
        .map(|d| project::load_settings(&d).editor_exe_name)
        .ok()
        .filter(|s| !s.trim().is_empty())
        .unwrap_or_else(|| editor::DEFAULT_EDITOR_EXE.to_string())
}

/// 目标项目在线端口（不在线给引导性错误）
fn require_online(project: &Path) -> Result<(editor::EditorTarget, u16)> {
    let target = editor::locate(project)?;
    editor::online_port(&target).map(|p| (target, p)).ok_or_else(|| {
        anyhow!("编辑器不在线（MCP 桥不可达）。请先 editor_start 启动编辑器")
    })
}
...
fn tool_editor_start(args: &Value) -> Result<Value> {
    ...
    let timeout = args
        ...
        .and_then(|v| v.as_u64())
        .unwrap_or(120_000);
    editor::editor_start(&project, &editor_exe_name(), wait, timeout)
}

fn tool_editor_stop(args: &Value) -> Result<Value> {
    let project = resolve_project(Some(args))?;
    editor::editor_stop(&project, &editor_exe_name())
}
...
fn tool_get_logs(args: &Value) -> Result<Value> {
    ...
    editor::get_logs(&project, source, tail)
}
...
fn tool_start_debug(args: &Value) -> Result<Value> {
    ...
    if full {
        return editor::bridge_rpc(port, "start_debug", json!({}), DEBUG_TIMEOUT_MS);
    }
    // 默认 restart_last_debug（跳过编辑器编译构建、载入最新 lua）；无上一次调试版本时回退全量
    match editor::bridge_rpc(port, "restart_last_debug", json!({}), DEBUG_TIMEOUT_MS) {
        Ok(v) => Ok(v),
        Err(e) => {
            let fallback = editor::bridge_rpc(port, "start_debug", json!({}), DEBUG_TIMEOUT_MS)?;
            Ok(json!({
...
fn tool_stop_debug(args: &Value) -> Result<Value> {
    let project = resolve_project(Some(args))?;
    let (_t, port) = require_online(&project)?;
    editor::bridge_rpc(port, "stop_debug", json!({}), 30_000)
}

fn tool_get_status(args: &Value) -> Result<Value> {
    let project = resolve_project(Some(args))?;
    let (_t, port) = require_online(&project)?;
    editor::bridge_rpc(port, "get_status", json!({}), 15_000)
}
...
fn tool_publish_project(args: &Value) -> Result<Value> {
    ...
    let timeout = args
        ...
        .and_then(|v| v.as_u64())
        .unwrap_or(600_000);
    editor::bridge_invoke(port, "lua.publish_project", json!({}), timeout)
}
...
fn tools_list() -> Value {
    json!({
        "tools": [
            {"name":"editor_start","description":"启动星火编辑器（按项目组装启动命令，等待 MCP 桥上线；幂等：已在线直接返回）","inputSchema":{"type":"object","properties":{"project_path":{"type":"string","description":"项目根，缺省取 bgd_sce_tools 最近项目"},"wait_online":{"type":"boolean","default":true},"timeout_ms":{"type":"integer","default":120000}}}},
            ...
            {"name":"capture_game","description":"截取调试中的游戏画面，返回 png 路径（落盘 <项目>/.bgd/log/screenshots/），用 Read 查看","inputSchema":{"type":"object","properties":{"project_path":{"type":"string"}}}},
    ...
    match method {
        "initialize" => Some(result_ok(
            ...
            json!({
                ...
                "capabilities": { "tools": {} },
                "serverInfo": { "name": "bgd_sce_tools", "version": env!("CARGO_PKG_VERSION") }
            }),
        )),
