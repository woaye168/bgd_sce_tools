...
8. **资源系统**：`res/` 目录五类资源（image/particle/sound/spine/sprites）同步到引擎目录；`.lua` 中 `'libs/res/<类型>/...'` / `'src/res/<类型>/...'` 构建时替换为运行时路径（sound 去 `.ogg`，sprites 前缀 `@<ProjectName>`）；资源类型/落位/运行时前缀均为「设置-构建路径配置」里的资源路径规则（`res_rules`），可按项目覆盖
